...
dag_1 = DAG('write_to_xcom', schedule_interval=None, start_date=datetime(2020, 12, 1), tags=['avikm'])


def push_xcom_call(**kwargs):
    kwargs['ti'].xcom_push(key='test_dag', value=str({'key1': 'value1'}))
...
dag_2 = DAG('read_from_xcom', schedule_interval=None, start_date=datetime(2020, 12, 3), tags=['avikm'])
...
def pull_xcom_call(**kwargs):
    ...
    get_mane_xcoms_values__with_xcom_class = XCom.get_many(
        execution_date=make_aware(datetime(2021, 2, 7, 20, 40, 58, 00)),
    ...
    print('task_instance.xcom_pull with include_prior_dates')
